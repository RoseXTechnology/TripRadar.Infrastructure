...
# Libby Container App (optional)
resource "azurerm_container_app" "libby" {
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.libby.id]
  }
  ...
  dynamic "registry" {
    ...
    content {
      ...
      identity = azurerm_user_assigned_identity.libby.id
    }
  }

  dynamic "ingress" {
    for_each = var.libby_ingress_external ? [1] : []
    content {
      external_enabled           = var.libby_ingress_external
      target_port                = var.libby_port
      transport                  = "auto"
  ...
  secret {
    ...
    identity            = local.kv_postgres_secret_id != null ? azurerm_user_assigned_identity.libby.id : null
  }

  secret {
    name                = "redis-conn"
    key_vault_secret_id = local.kv_redis_secret_id
    identity            = local.kv_redis_secret_id != null ? azurerm_user_assigned_identity.libby.id : null
  }

  secret {
    name                = "appi-conn"
    key_vault_secret_id = local.kv_appi_secret_id
    identity            = local.kv_appi_secret_id != null ? azurerm_user_assigned_identity.libby.id : null
  ...
  template {
    ...
    container {
      ...
      env {
        ...
        value = "http://0.0.0.0:${var.libby_port}"
      }
